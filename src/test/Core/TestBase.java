...
public class TestBase {
	...
	public static void beforeTest(@Optional String Browser) {
		...
		if (ReadProperties.Properties("Mobile").equals("yes")) {
			try {
				DesiredCapabilities capabilities = new DesiredCapabilities();
				capabilities.setCapability("deviceName", ReadProperties.Properties("MobiledeviceName"));
				capabilities.setCapability("platformName", ReadProperties.Properties("MobileplatformName"));
				capabilities.setCapability("appPackage", ReadProperties.Properties("MobileappPackage"));
				capabilities.setCapability("appActivity", ReadProperties.Properties("MobileappActivity"));
				capabilities.setCapability("platformName", ReadProperties.Properties("MobileplatformName"));
				log.InfoMobile(capabilities.getVersion());
				mobile_driver = new AndroidDriver<>(new URL("http://127.0.0.1:4723/wd/hub"),capabilities);
				log.InfoMobile("Mobile session was successfully created");
				new MobileAction(mobile_driver);
				
				
		         		
			} catch (Exception e) {
				// TODO: handle exception
				System.err.println("Please check Emulator settings.");
				mobile_driver.quit();
	...
	@AfterMethod(alwaysRun = true)
	  public static void afterMethod()
	  {
		...
		if (ReadProperties.Properties("Mobile").toLowerCase().equals("yes")) {
